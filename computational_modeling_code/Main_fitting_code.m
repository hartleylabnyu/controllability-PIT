...
% The script starts by defining the working directory where the code and data reside. 
% It then navigates to this directory.

% 2. Loading Behavioral Data
% The behavioral data from a specified directory is loaded into MATLAB and converted 
% into a suitable data structure for further analysis.

% 3. Model Fitting
% Models are fit to the behavioral data. The result includes parameters, BIC values, 
% and other metrics.
...
% 5. BIC Analysis
% Mean BIC values are calculated for each dataset and then plotted. 
% The plotted BIC values can be used to compare the fit of different models.

% 6. Model Recovery
% The script simulates parameters for different models, aiming to check if parameters 
% can be recovered from data that's generated using known parameters.

% 7. Scatter Plots for Parameter Recovery
% Scatter plots are used to visually compare generated (true) parameters with 
% parameters estimated from the data.

% 8. Weight (w) Analysis per Trial
% For each subject and trial, a weight (referred to as 'w') is estimated 
% and saved to a CSV file.

% 9. Posterior Predictive Checks
% The script appears to create data for posterior predictive checks. These checks can 
% be used to validate the fit of a model by simulating new data from the model's 
% posterior and comparing it to the observed data.

% 10. Export Simulated Data
% The script simulates subject-specific datasets based on the parameters estimated 
% from the actual data. These datasets are then exported to individual CSV files.

% NOTE: Before running the script or sharing it, make sure to check the path to your 
% working directory and other sensitive information you might not want to share.

% -------------------------
% SETUP
% -------------------------

%WD='/path/to/computational_modeling_code'
addpath(WD)

cd(WD) %insert the correct directory
