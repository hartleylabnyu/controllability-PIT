function [data , latents] = sim_adaptive_gng_samemeanconfwinit2(param,data) %noam feb 7 2023
    %data = sim_adaptive_gng_samemeanconfwinitboth(param,data)
    ...
    w02 = param (4); %free  w after condition change
    %w02 = param (5);
    ...
    for n = 1:data.N
        ...
        if n == condition_change
            ...
            Mv = zeros(S,1) + pvq; % same for confidence
            Mq = zeros(S,2) + pvq;
            L = log(w02) - log(1-w02); %initialize L, L is odds of being in uncontrollable environment; if w0=0.5, then L0=0
            b = b2;
        end
