function [simulated_data, model_results] = pcc_run_param_recoverability_params_as_input(n_simulations,simulated_params,models)
...
%calculate correlations between the simulated and fitted parameters, but 
    ...
    % 3: NoGotoWin
    % 4: NoGotoAvoid
    simfuns = { 'sim_adaptive_gng_samemeanconffreewinit2'};
...
%         end
        ...
        for s = 1:n_simulations
            ...
            switch simfuns{m}
               ...
               case 'sim_adaptive_gng_samemeanconffreewinit2'
        ...
        %model_results(m).model = fit_models(simulated_data(m).behavior,m);
        temp = fit_models(simulated_data(m).behavior,models);
        model_results(m).model = temp;
        %model_results(m) = temp(m);

        %for n = 1:size(simulated_params(m).x,2)
        %    [correlations(m).r(n),correlations(m).p(n)] = corr(model_results(m).x(:,n),simulated_params(m).x(:,n));  
        %end
    end
